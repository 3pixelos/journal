...
/**
 * Checklist completion per day: { [date]: ratio 0..1 }.
 * `activeCount` is how many items are active today; historical days use the
 * number of items that were logged as the denominator when it is larger.
 */
export function checklistRatios(logs, activeCount) {
  const map = new Map()
  for (const l of logs) {
    if (!map.has(l.log_date)) map.set(l.log_date, 0)
    if (l.completed) map.set(l.log_date, map.get(l.log_date) + 1)
  }
  const out = {}
  for (const [date, done] of map) {
    out[date] = activeCount > 0 ? Math.min(done / activeCount, 1) : 0
  }
  return out
}

/** Consecutive days (walking back from today) with a full checklist. */
export function currentStreak(ratios, today) {
  let streak = 0
  let cursor = today
  // today only breaks the streak once it is actually complete-able; if today
  // is incomplete we start counting from yesterday instead.
  if ((ratios[cursor] ?? 0) < 1) cursor = addDays(cursor, -1)
  while ((ratios[cursor] ?? 0) >= 1) {
    streak += 1
...
export function longestStreak(ratios) {
  const days = Object.keys(ratios).filter((d) => ratios[d] >= 1).sort()
  let best = 0
  let run = 0
  let prev = null
  for (const d of days) {
...
/**
 * Does discipline pay? Compare days where the checklist was fully completed
 * against days where it was not.
 */
export function disciplineSplit(dayRows, ratios) {
  const done = []
  const missed = []
  for (const d of dayRows) {
    ;((ratios[d.date] ?? 0) >= 1 ? done : missed).push(d)
  }
  const agg = (rows) => ({
    days: rows.length,
    net: sum(rows, (r) => r.pnl),
    avg: rows.length ? sum(rows, (r) => r.pnl) / rows.length : 0,
    greenDays: rows.filter((r) => r.pnl > 0).length,
    winRate: rows.length ? rows.filter((r) => r.pnl > 0).length / rows.length : 0,
  })
  return { done: agg(done), missed: agg(missed) }
}
